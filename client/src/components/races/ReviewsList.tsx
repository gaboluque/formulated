...
import { RaceReviewForm } from './ReviewForm';
...
export interface RaceReviewsListProps {
    race: Race;
}

export const RaceReviewsList = ({ race }: RaceReviewsListProps) => {
    const { user } = useAuth();
    ...
    const loadReviews = async () => {
        try {
            setLoading(true);
            const reviewsData = await racesApi.getReviews(race.id);
    ...
    const handleSubmitReview = async (data: ReviewFormData) => {
        ...
        try {
            setSubmitting(true);
            if (editingReview) {
                await racesApi.updateReview(race.id, data);
            } else {
                await racesApi.createReview(race.id, data);
            }
            await loadReviews();
            setShowForm(false);
    ...
    const handleDeleteReview = async () => {
        ...
        try {
            await racesApi.deleteReview(race.id);
            await loadReviews();
        } catch (err) {
            ...
            <div className="flex">
                {[1, 2, 3, 4, 5].map((star) => (
                    <span
                        key={star}
                        className={`text-lg ${
                            star <= rating ? 'text-yellow-400' : 'opacity-50'
                        }`}
                    >
                        ⭐
                    </span>
    ...
    return (
        <div className="space-y-6">
            <div className="flex justify-between items-center">
                ...
                {user && !userReview && !showForm && isRaceFinished() && (
                    <Button
